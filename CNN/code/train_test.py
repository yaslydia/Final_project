...
create_tmp_dirs(
    ['/home/cxyycl/scratch/Microsleep-code/code/models/', '/home/cxyycl/scratch/Microsleep-code/code/predictions/'])
...
for i in range(n_ep):
    ...
    for j in range(len(data_val)):
        ...
        scores = model.evaluate_generator(generator_val, int(math.ceil((len(sample_list_val[j], ) + 0.0) / batch_size)),
                                          workers=1)
        generator_val = my_generator(data_val, targets_val, sample_list_val[j], shuffle=False)
        y_pred = model.predict_generator(generator_val, int(math.ceil((len(sample_list_val[j], ) + 0.0) / batch_size)),
                                         workers=1)
    ...
    spio.savemat('/home/cxyycl/scratch/Microsleep-code/code/predictions/predictions0_ep' + str(i) + '.mat',
...
spio.savemat('/home/cxyycl/scratch/Microsleep-code/code/predictions/predictions0.mat',
             mdict={'val_y': val_y, 'val_y_': val_y_, 'val_l': val_l, 'files_test': files_test, 'files_val': files_val,
                    'acc_tr': acc_tr, 'loss_tr': loss_tr, 'loss_val': loss_val, 'acc_val': acc_val, 'K_val': K_val})


# 保存 cnn_eeg 模型
cnn_eeg.save('/home/cxyycl/scratch/Microsleep-code/code/predictions/cnn_eeg_model.h5')

# 保存 model 模型
model.save('/home/cxyycl/scratch/Microsleep-code/code/predictions/full_model.h5')
